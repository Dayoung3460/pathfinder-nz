...
This file provides context and guidance for Codex when working on the Pathfinder NZ project.
...
Agents are defined in `.codex/agents/`. **Always automatically select and use the most appropriate agent(s) for each task. Never wait for the user to specify which agent to use.**
